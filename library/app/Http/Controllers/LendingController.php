...
class LendingController extends Controller
{
    ...
    public function lendingFirst($title, $author, $copy_id)
    {
        $user = Auth::user();
        $recordExists = DB::table('lendings as l')
            ->join('copies as c', 'l.copy_id', '=', 'c.copy_id')
            ->join('books as b', 'c.book_id', '=', 'b.book_id')
    ...
    public function noticeInsert($copy_id)
    {
        $user = Auth::user();
        $figyelmeztetesek = DB::table('lendings')
            ->where('user_id', $user->id)
            ->sum('notice');
        if ($figyelmeztetesek <= 3) {
            DB::table('lendings')->insert([
                'user_id' => $user->id,
                'copy_id' => $copy_id,
                'start' => date(now()),
